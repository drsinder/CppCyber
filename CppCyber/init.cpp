...
static void initCyber(char *config)
{
	...
	*/
	...
	(void)initGetInteger("cpus", 1, &initCpus);	//drs
	
#ifndef DUAL_CPU
	if (initCpus != 1)
	{
		printf("Program not compiled for multiple CPUs - using 1 CPU\n");
		initCpus = 1;
	}
#else
	if (initCpus > 2)
	{ 
		printf("Too many CPUs specified.  Setting to 2.\n");
		initCpus = 2;
	}
	if (initCpus < 1)
	{
	...
	**  Initialise CPU(s).
	*/
	for (int i = 0; i < initCpus; i++)
	{
		Acpu[i] = new MCpu(i);
		Acpu[i]->Init(model, memory, ecsBanks + esmBanks, ecsBanks != 0 ? ECS : ESM);
	}
