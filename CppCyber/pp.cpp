...
void ppInit(u8 count)
{
	...
	*/
	ppuCount = count;
	ppu = (PpSlot*)calloc(count, sizeof(PpSlot));
	if (ppu == NULL)
	{
...
static void ppOpEXN(void)     // 26
{
	...
	*/
	...
	RESERVE(&PreXJMutex)
	while (!cpu->ExchangeJump(exchangeAddress))
	{
		cpu->Step();
	}
	RELEASE(&PreXJMutex)
...
static void ppOpRPN(void)     // 27
{
	...
	*/
	if ((features & IsSeries800) == 0 || modelType == ModelCyber865)
	{
		...
		MCpu *cpu = (opD & 1) == 0 ? cpu0 : cpu1;  // DRS??

		activePpu->regA = cpu->GetP();
	}
}
...
static void ppOpCRD(void)     // 60
{
	...
	if ((activePpu->regA & Sign18) != 0 && (features & HasRelocationReg) != 0)
	{
		cpu->PpReadMem(activePpu->regR + (activePpu->regA & Mask17), &data);
	}
	else
	{
		cpu->PpReadMem(activePpu->regA & Mask18, &data);
	}
...
static void ppOpCWD(void)     // 62
{
	...
	if ((activePpu->regA & Sign18) != 0 && (features & HasRelocationReg) != 0)
	{
		cpu->PpWriteMem(activePpu->regR + (activePpu->regA & Mask17), data);
	}
	else
	{
		cpu->PpWriteMem(activePpu->regA & Mask18, data);
	}
}
